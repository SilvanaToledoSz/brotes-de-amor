let bd = "http://localhost:3000/base"
let bdComp = "http://localhost:3000/compradores"

let listadoPlantas = document.getElementById("listadoPlantas")
let btnComprarHome = document.getElementById("btnComprarHome") 
let carritoHeader = document.getElementById("carritoHeader")
let mostrarReloj = document.getElementById("hora")
let spinnerHome = document.getElementById("spinnerHome")
let contadorProd = document.getElementById("contadorProd")
let listadoCarrito = document.getElementById("listadoCarrito")
let valorTotal = document.getElementById("valorTotal")
let divComprar = document.getElementById("divComprar")
let divVolver = document.getElementById("divVolver")
let formu = document.getElementById("formu")
let recuento = document.getElementById("recuento")
let totalPagar = document.getElementById("totalPagar")
let submit = document.querySelector("#submit")
let graciasVolver = document.getElementById("graciasVolver")
const DateTime = luxon.DateTime
let fechaHoy = document.getElementById("fechaHoy")
let fechaEntrega = document.getElementById("fechaEntrega")
let masDatos = document.getElementById("masDatos")


